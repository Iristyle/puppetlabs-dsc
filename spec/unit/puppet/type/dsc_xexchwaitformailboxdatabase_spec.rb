...
describe Puppet::Type.type(:dsc_xexchwaitformailboxdatabase) do
  ...
  it 'should accept array for dsc_credential' do
    dsc_xexchwaitformailboxdatabase[:dsc_credential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xexchwaitformailboxdatabase[:dsc_credential]).to eq({"user"=>"user", "password"=>"password"})
  end
