...
describe Puppet::Type.type(:dsc_xspdistributedcacheservice) do
  ...
  it 'should accept array for dsc_installaccount' do
    dsc_xspdistributedcacheservice[:dsc_installaccount] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xspdistributedcacheservice[:dsc_installaccount]).to eq({"user"=>"user", "password"=>"password"})
  end
