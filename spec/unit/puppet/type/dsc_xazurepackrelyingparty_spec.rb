...
describe Puppet::Type.type(:dsc_xazurepackrelyingparty) do
  ...
  it 'should accept array for dsc_azurepackadmincredential' do
    dsc_xazurepackrelyingparty[:dsc_azurepackadmincredential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xazurepackrelyingparty[:dsc_azurepackadmincredential]).to eq({"user"=>"user", "password"=>"password"})
  end
