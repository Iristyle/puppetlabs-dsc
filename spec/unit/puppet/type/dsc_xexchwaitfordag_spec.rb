...
describe Puppet::Type.type(:dsc_xexchwaitfordag) do

  let :dsc_xexchwaitfordag do
    Puppet::Type.type(:dsc_xexchwaitfordag).new(
      :name     => 'foo',
      :dsc_identity => 'foo',
    )
  end
  ...
  it 'should not accept array for dsc_identity' do
    expect{dsc_xexchwaitfordag[:dsc_identity] = ["foo", "bar", "spec"]}.to raise_error(Puppet::ResourceError)
  end

  it 'should not accept boolean for dsc_identity' do
    expect{dsc_xexchwaitfordag[:dsc_identity] = true}.to raise_error(Puppet::ResourceError)
  end
  ...
  it 'should accept array for dsc_credential' do
    dsc_xexchwaitfordag[:dsc_credential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xexchwaitfordag[:dsc_credential]).to eq({"user"=>"user", "password"=>"password"})
  end

  it 'should not accept boolean for dsc_credential' do
    expect{dsc_xexchwaitfordag[:dsc_credential] = true}.to raise_error(Puppet::ResourceError)
  end
  ...
  it 'should not accept boolean for dsc_domaincontroller' do
    expect{dsc_xexchwaitfordag[:dsc_domaincontroller] = true}.to raise_error(Puppet::ResourceError)
  end
  ...
  describe "powershell provider tests" do
    ...
    it "should successfully instanciate the provider" do
      described_class.provider(:powershell).new(dsc_xexchwaitfordag)
    end

    before(:each) do
      @provider = described_class.provider(:powershell).new(dsc_xexchwaitfordag)
    end
