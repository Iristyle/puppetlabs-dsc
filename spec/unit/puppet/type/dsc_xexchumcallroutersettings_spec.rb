...
describe Puppet::Type.type(:dsc_xexchumcallroutersettings) do
  ...
  it 'should accept array for dsc_credential' do
    dsc_xexchumcallroutersettings[:dsc_credential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xexchumcallroutersettings[:dsc_credential]).to eq({"user"=>"user", "password"=>"password"})
  end
