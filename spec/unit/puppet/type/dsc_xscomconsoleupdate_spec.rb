...
describe Puppet::Type.type(:dsc_xscomconsoleupdate) do
  ...
  it 'should accept array for dsc_setupcredential' do
    dsc_xscomconsoleupdate[:dsc_setupcredential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xscomconsoleupdate[:dsc_setupcredential]).to eq({"user"=>"user", "password"=>"password"})
  end
