...
describe Puppet::Type.type(:dsc_xspusageapplication) do
  ...
  it 'should accept array for dsc_installaccount' do
    dsc_xspusageapplication[:dsc_installaccount] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xspusageapplication[:dsc_installaccount]).to eq({"user"=>"user", "password"=>"password"})
  end
