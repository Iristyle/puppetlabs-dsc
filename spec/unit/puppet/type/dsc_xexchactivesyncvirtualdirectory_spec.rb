...
describe Puppet::Type.type(:dsc_xexchactivesyncvirtualdirectory) do
  ...
  it 'should accept array for dsc_credential' do
    dsc_xexchactivesyncvirtualdirectory[:dsc_credential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xexchactivesyncvirtualdirectory[:dsc_credential]).to eq({"user"=>"user", "password"=>"password"})
  end
