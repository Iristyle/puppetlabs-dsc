...
describe Puppet::Type.type(:dsc_xexchclientaccessserver) do
  ...
  it 'should accept array for dsc_credential' do
    dsc_xexchclientaccessserver[:dsc_credential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xexchclientaccessserver[:dsc_credential]).to eq({"user"=>"user", "password"=>"password"})
  end
