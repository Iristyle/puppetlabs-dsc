...
describe Puppet::Type.type(:dsc_xwordpresssite) do
  ...
  it 'should accept array for dsc_administratorcredential' do
    dsc_xwordpresssite[:dsc_administratorcredential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xwordpresssite[:dsc_administratorcredential]).to eq({"user"=>"user", "password"=>"password"})
  end
