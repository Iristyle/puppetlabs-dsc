...
describe Puppet::Type.type(:dsc_xexchdatabaseavailabilitygroupmember) do
  ...
  it 'should accept array for dsc_credential' do
    dsc_xexchdatabaseavailabilitygroupmember[:dsc_credential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xexchdatabaseavailabilitygroupmember[:dsc_credential]).to eq({"user"=>"user", "password"=>"password"})
  end
