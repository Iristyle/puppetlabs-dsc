...
describe Puppet::Type.type(:dsc_xdatabaselogin) do
  ...
  it 'should accept array for dsc_sqlconnectioncredential' do
    dsc_xdatabaselogin[:dsc_sqlconnectioncredential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xdatabaselogin[:dsc_sqlconnectioncredential]).to eq({"user"=>"user", "password"=>"password"})
  end
