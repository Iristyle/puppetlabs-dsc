...
describe Puppet::Type.type(:dsc_xexchmailboxdatabasecopy) do
  ...
  it 'should accept array for dsc_credential' do
    dsc_xexchmailboxdatabasecopy[:dsc_credential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xexchmailboxdatabasecopy[:dsc_credential]).to eq({"user"=>"user", "password"=>"password"})
  end
