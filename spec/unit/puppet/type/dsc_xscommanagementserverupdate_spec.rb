...
describe Puppet::Type.type(:dsc_xscommanagementserverupdate) do
  ...
  it 'should accept array for dsc_setupcredential' do
    dsc_xscommanagementserverupdate[:dsc_setupcredential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xscommanagementserverupdate[:dsc_setupcredential]).to eq({"user"=>"user", "password"=>"password"})
  end
