...
describe Puppet::Type.type(:dsc_xscommanagementpack) do
  ...
  it 'should accept array for dsc_scomadmincredential' do
    dsc_xscommanagementpack[:dsc_scomadmincredential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xscommanagementpack[:dsc_scomadmincredential]).to eq({"user"=>"user", "password"=>"password"})
  end
