...
describe Puppet::Type.type(:dsc_xscomconsolesetup) do
  ...
  it 'should accept array for dsc_setupcredential' do
    dsc_xscomconsolesetup[:dsc_setupcredential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xscomconsolesetup[:dsc_setupcredential]).to eq({"user"=>"user", "password"=>"password"})
  end
