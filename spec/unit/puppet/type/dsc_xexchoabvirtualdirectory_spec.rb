...
describe Puppet::Type.type(:dsc_xexchoabvirtualdirectory) do
  ...
  it 'should accept array for dsc_credential' do
    dsc_xexchoabvirtualdirectory[:dsc_credential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xexchoabvirtualdirectory[:dsc_credential]).to eq({"user"=>"user", "password"=>"password"})
  end
