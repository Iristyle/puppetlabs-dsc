...
describe Puppet::Type.type(:dsc_xazurepackadmin) do
  ...
  it 'should accept array for dsc_azurepackadmincredential' do
    dsc_xazurepackadmin[:dsc_azurepackadmincredential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xazurepackadmin[:dsc_azurepackadmincredential]).to eq({"user"=>"user", "password"=>"password"})
  end
