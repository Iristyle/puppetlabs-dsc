...
describe Puppet::Type.type(:dsc_xscvmmmanagementserverupdate) do
  ...
  it 'should accept array for dsc_setupcredential' do
    dsc_xscvmmmanagementserverupdate[:dsc_setupcredential] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xscvmmmanagementserverupdate[:dsc_setupcredential]).to eq({"user"=>"user", "password"=>"password"})
  end
