...
describe Puppet::Type.type(:dsc_xspserviceinstance) do
  ...
  it 'should accept array for dsc_installaccount' do
    dsc_xspserviceinstance[:dsc_installaccount] = {"user"=>"user", "password"=>"password"}
    expect(dsc_xspserviceinstance[:dsc_installaccount]).to eq({"user"=>"user", "password"=>"password"})
  end
