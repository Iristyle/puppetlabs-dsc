...
Puppet::Type.newtype(:dsc_xcomputer) do
  ...
  newparam(:dsc_unjoincredential) do
    ...
    validate do |value|
      ...
      missing = required - value.keys.map(&:to_s)
      unless missing.empty?
        fail "for UnjoinCredential you are missing the following keys: #{missing.join(',')}"
      end
      required.each do |key|
        if value[key]
          fail "#{key} for UnjoinCredential should be a String" unless value[key].is_a? String
        end
      end
    end
  end
