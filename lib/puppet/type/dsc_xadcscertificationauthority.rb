...
Puppet::Type.newtype(:dsc_xadcscertificationauthority) do
  ...
  newparam(:dsc_certfilepassword) do
    ...
    validate do |value|
      ...
      missing = required - value.keys.map(&:to_s)
      unless missing.empty?
        fail "for CertFilePassword you are missing the following keys: #{missing.join(',')}"
      end
      required.each do |key|
        if value[key]
          fail "#{key} for CertFilePassword should be a String" unless value[key].is_a? String
        end
      end
    end
  end
