...
Puppet::Type.newtype(:dsc_file) do
  ...
  newparam(:dsc_psdscrunascredential) do
    ...
    validate do |value|
      ...
      missing = required - value.keys.map(&:to_s)
      unless missing.empty?
        fail "for PsDscRunAsCredential you are missing the following keys: #{missing.join(',')}"
      end
      required.each do |key|
        if value[key]
          fail "#{key} for PsDscRunAsCredential should be a String" unless value[key].is_a? String
        end
      end
    end
  end
