...
Puppet::Type.newtype(:dsc_xmysqluser) do
  ...
  newparam(:dsc_connectioncredential) do
    ...
    validate do |value|
      ...
      missing = required - value.keys.map(&:to_s)
      unless missing.empty?
        fail "for ConnectionCredential you are missing the following keys: #{missing.join(',')}"
      end
      required.each do |key|
        if value[key]
          fail "#{key} for ConnectionCredential should be a String" unless value[key].is_a? String
        end
      end
    end
  end
