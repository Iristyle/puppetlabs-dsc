...
Puppet::Type.newtype(:dsc_xspstateserviceapp) do
  ...
  newparam(:dsc_databasecredentials) do
    ...
    validate do |value|
      ...
      unless missing.empty?
        fail "for DatabaseCredentials you are missing the following keys: #{missing.join(',')}"
      end
      required.each do |key|
        if value[key]
          fail "#{key} for DatabaseCredentials should be a String" unless value[key].is_a? String
        end
      end
    end
  end
