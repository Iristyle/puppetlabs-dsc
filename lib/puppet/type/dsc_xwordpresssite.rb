  ...
  newparam(:dsc_administratorcredential) do
    ...
    validate do |value|
      ...
      unless missing.empty?
        fail "for AdministratorCredential you are missing the following keys: #{missing.join(',')}"
      end
      required.each do |key|
        if value[key]
          fail "#{key} for AdministratorCredential should be a String" unless value[key].is_a? String
        end
      end
    end
  end
