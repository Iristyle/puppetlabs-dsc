...
Puppet::Type.newtype(:dsc_xscommanagementserversetup) do
  ...
  newparam(:dsc_actionaccount) do
    ...
    validate do |value|
      ...
      missing = required - value.keys.map(&:to_s)
      unless missing.empty?
        fail "for ActionAccount you are missing the following keys: #{missing.join(',')}"
      end
      required.each do |key|
        if value[key]
          fail "#{key} for ActionAccount should be a String" unless value[key].is_a? String
        end
      end
    end
  end
  ...
  newparam(:dsc_dasaccount) do
    ...
    validate do |value|
      ...
      missing = required - value.keys.map(&:to_s)
      unless missing.empty?
        fail "for DASAccount you are missing the following keys: #{missing.join(',')}"
      end
      required.each do |key|
        if value[key]
          fail "#{key} for DASAccount should be a String" unless value[key].is_a? String
        end
      end
    end
  end
  ...
  newparam(:dsc_datareader) do
    ...
    validate do |value|
      ...
      missing = required - value.keys.map(&:to_s)
      unless missing.empty?
        fail "for DataReader you are missing the following keys: #{missing.join(',')}"
      end
      required.each do |key|
        if value[key]
          fail "#{key} for DataReader should be a String" unless value[key].is_a? String
        end
      end
    end
  end
  ...
  newparam(:dsc_datawriter) do
    ...
    validate do |value|
      ...
      missing = required - value.keys.map(&:to_s)
      unless missing.empty?
        fail "for DataWriter you are missing the following keys: #{missing.join(',')}"
      end
      required.each do |key|
        if value[key]
          fail "#{key} for DataWriter should be a String" unless value[key].is_a? String
        end
      end
    end
  end
