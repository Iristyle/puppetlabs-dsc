  ...
  newparam(:dsc_sqlconnectioncredential) do
    ...
    validate do |value|
      ...
      unless missing.empty?
        fail "for SqlConnectionCredential you are missing the following keys: #{missing.join(',')}"
      end
      required.each do |key|
        if value[key]
          fail "#{key} for SqlConnectionCredential should be a String" unless value[key].is_a? String
        end
      end
    end
  end
